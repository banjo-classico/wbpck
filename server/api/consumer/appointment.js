...
const getAvailableTimeSlots = (numItems, day) => {
  const randomDays = Math.floor(Math.random() * 10) + 1;
  const randomGuid = () => Math.random().toString()
  const items = compose(
    reduce((sum, next) => ({...sum, [next.Guid]: next.Time}), {}),
    times((i) => ({"Guid": randomGuid(), "Time": moment(day).add(8, "hours").add(i * 30, "minutes")}))
  )(numItems)
  const shouldReturn = !moment("2017-02-24").isSame(day, 'day')
...
let data = (day) => [
  {
    ...
    AvailableSlots: getAvailableTimeSlots(26, day),
    NextAvailableSlot: moment().add(10,'days'),
  },{
    PmsUserId: "2",
    ...
    AvailableSlots: getAvailableTimeSlots(26, day),
    NextAvailableSlot: moment().add(9,'days'),
  },{
    PmsUserId: "3",
    Type: "Oldest Child",
    Name: "Tina Belcher",
    ...
    AvailableSlots: [],
    NextAvailableSlot: moment().add(8,'days').add(8, "hours"),
  },{
    PmsUserId: "4",
    ...
    AvailableSlots: getAvailableTimeSlots(Math.floor(Math.random() *  30), day),
    NextAvailableSlot: moment().add(6,'days'),
  },{
    PmsUserId: "6",
    ...
    AvailableSlots: getAvailableTimeSlots(Math.floor(Math.random() *  40), day),
    NextAvailableSlot: moment().add(3,'days'),
  }
]
